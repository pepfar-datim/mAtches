...
class UploadMapList extends Component {
  ...
  getMaps() {
    api.get("api/maps").then((maps) => {
      this.setState({
        maps: maps,
        maxSteps: Math.ceil(maps.length / this.state.numberOfMaps),
      });
    });
  }
  ...
  handleSearchTextChange(newText) {
    ...
    if (newText.trim().length != 0) {
      var tempLength = this.state.maps.filter((m) => {
        return m.name.toLowerCase().includes(newText.trim().toLowerCase());
      }).length;
    ...
    return (
      <div style={stylesObj.themePadding}>
        ...
        {formatMapList(
          ...
          this.props.id,
          this.props.onMapProcess,
          this.state.currentStep,
          this.state.numberOfMaps
        )}
        <MobileStepper
          ...
          nextButton={
            <Button
              ...
              disabled={this.state.currentStep === this.state.maxSteps - 1}
            >
              <KeyboardArrowRight />
              Next
            </Button>
