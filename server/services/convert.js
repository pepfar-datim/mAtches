...
var data = {
  ...
  addInConstants: function (row, QR, pathsChecked) {
    Object.keys(this.map.constants).forEach((key) => {
      let tempValue = {
        value: this.map.constants[key].code,
        valueType: this.map.constants[key].valueType,
      };
    ...
    for (let i = 0; i < this.csvData.length; i++) {
      ...
      Object.keys(this.csvData[i]).forEach((key) => {
        if (this.map.headers.hasOwnProperty(key)) {
          var tempValue = {
            valueType: this.map.headers[key].valueType,
            value: this.csvData[i][key],
          };
          var requiredItem =
            this.map.headers[key].path[this.map.headers[key].path.length - 1]
              .required || false;
  ...
  convertValue: function (valueObject, valueChoiceMap, row, key) {
    ...
    try {
      ...
      return {
        value: valueChoiceMap[valueObject.value].code,
        valueType: valueChoiceMap[valueObject.value].valueType,
      };
    } catch (e) {
